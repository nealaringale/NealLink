...
TOGGLE_HOTKEY = {keyboard.Key.ctrl_l, keyboard.Key.alt_l, keyboard.Key.space}
TOGGLE_HOTKEY_ALIASES = {
    keyboard.Key.ctrl,
    keyboard.Key.ctrl_l,
    keyboard.Key.ctrl_r,
    keyboard.Key.alt,
...
class UbuntuAndroidBridge:
    """Share one physical mouse/keyboard between Ubuntu and Android via a toggle hotkey."""
    ...
    def _warp_mouse(self, x: int, y: int) -> None:
        """Place the real Ubuntu pointer at a known location without creating a feedback loop."""
        self.ignore_mouse_until = asyncio.get_event_loop_policy().get_event_loop().time() if False else 0.0
        try:
            subprocess.run(
    ...
    def toggle_tablet_mode(self) -> None:
        with self.state_lock:
            ...
            if entering:
                ...
                self.tablet_y = self.tablet_height // 2
                try:
                    # Save the exact Ubuntu location before recentering.
                    current = subprocess.check_output(
                        ["xdotool", "getmouselocation"], text=True
                    )
                    for token in current.strip().split():
    ...
    def _check_toggle_combo(self, key: object, pressed: bool) -> bool:
        ...
        if combo_active and not self.toggle_latch:
    ...
    async def handler(self, websocket: ServerConnection) -> None:
        ...
        print("Toggle tablet control: Ctrl+Alt+Space")
